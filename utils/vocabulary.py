...
class Vocabulary:
    ...
    def build_vocabulary(self, caption_series):
        ...
        for caption in caption_series:
            word_counts.update(self.tokenize(caption) )
        
        sorted_words = sorted(word_counts.items(), key=lambda x: x[1], reverse=True)
        
        filtered_words = [word for word, count in sorted_words if count >= self.freq_threshold]
        
        # If max_size is specified, truncate to keep only top max_size words
        ...
        for idx, word in enumerate(filtered_words):
            self.word2idx[word] = idx + special_tokens
            self.idx2word[idx + special_tokens ] = word
