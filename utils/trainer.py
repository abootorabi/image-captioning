...
class CaptionTrainer:
    ...
    def train_epoch(self):
        ...
        for images , captions in self.train_loader : 
            ...
            outputs , _ = self.model(images, captions[:, :-1])  # assuming captions shifted for teacher forcing
            targets = captions[:, 1:]  # next word in sequence ; target
    ...
    def validate(self, generate_captions=False):
        ...
        with torch.no_grad():
            progress_bar = tqdm(enumerate(self.val_loader), total=len(self.val_loader), desc=f"Validation Epoch {self.current_epoch+1}")
            ...
            for batch_idx, (images, captions) in progress_bar:
                ...
                # Forward pass
                outputs , _ = self.model(images, captions[:, :-1])
                targets = captions[:, 1:]
        ...
        # If caption generation is enabled, compute BLEU score
        if generate_captions:
            bleu_score = self.calculate_metrics(num_samples= None  ) 
